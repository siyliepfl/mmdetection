...
class HDF5Backend(BaseStorageBackend):
    ...
    def __init__(self, img_db_path="", vid_db_path="", type='tao', **kwargs):
        # h5 file path
    ...
    def get(self, filepath):
        ...
        if self.type=='tao':
            ...
            value_buf = np.array(self.img_client[key_list[-4]][key_list[-3]][key_list[-2]][key_list[-1]])
        elif self.type == 'key':
            if self.img_client is None:
                self.img_client = h5py.File(self.img_db_path, 'r')
            ...
            if self.img_client is None:
                self.img_client = h5py.File(self.img_db_path, 'r')
            key_list = filepath.split('/')
            value_buf = np.array(self.img_client[key_list[-4]][key_list[-3]][key_list[-2]][key_list[-1]]['raw'])[0]
        elif self.type == 'bdd':
            filefolder, filename = os.path.split(filepath)
